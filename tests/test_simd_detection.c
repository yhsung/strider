...
#if defined(__x86_64__) || defined(_M_X64)
    strider_cpu_features_t features = strider_get_cpu_features();
    TEST_ASSERT_TRUE_MESSAGE(features.has_sse2, "SSE2 should be available on all x86_64 CPUs");
#else
...
#    ifdef __AVX2__
    ...
    TEST_ASSERT_TRUE_MESSAGE(features.has_avx2, "AVX2 compiled but not detected");
#    endif
#else
...
#    ifdef __AVX512F__
    TEST_ASSERT_TRUE_MESSAGE(features.has_avx512f, "AVX-512 compiled but not detected");
#    endif
#else
...
void test_cpu_has_neon(void) {
    ...
    strider_cpu_features_t features = strider_get_cpu_features();
    TEST_ASSERT_TRUE_MESSAGE(features.has_neon, "NEON should be available on all ARM64 CPUs");
#else
